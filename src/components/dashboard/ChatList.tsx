...
interface ChatListProps {
  ...
  filter: "telegram" | "discord" | "all";
}

const ChatList = ({ chats, activeChat, onChatSelect, filter }: ChatListProps) => {
  const filteredChats = filter === "all" ? chats : chats.filter((c) => c.platform === filter);
            ...
            <div className="relative">
              ...
              >
                {chat.platform === "telegram" ? (
                  <TelegramIcon className="w-3 h-3 text-primary-foreground" />
                ) : (
                  <DiscordIcon className="w-3 h-3 text-foreground" />
                )}
              </div>
            </div>
